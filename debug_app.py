...
@app.route('/')
def home():
    return jsonify({"message": "Welcome to the Bill and Executive Order Q&A API"})
...
def list_executive_orders():
    """List all available executive orders in the collection."""
    try:
        # Use scroll to retrieve all points with order_id in payload
        print("Retrieving executive orders...")
        ...
        order_ids_list = sorted(list(order_ids))
        print(f"Found {len(order_ids_list)} executive orders")
        
        return jsonify({
            "order_ids": order_ids_list
        })
    except Exception as e:
        print(f"Error in list_executive_orders: {str(e)}")
